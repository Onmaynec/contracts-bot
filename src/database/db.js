...
db.serialize(() => {
  ...
  // =========================
  // 🔧 МИГРАЦИЯ: Добавляем колонку table_type если её нет
  // =========================
  db.all("PRAGMA table_info(users)", [], (err, columns) => {
    ...
    if (!columnNames.has('table_type')) {
      db.run('ALTER TABLE users ADD COLUMN table_type TEXT DEFAULT "small"', (err) => {
        if (err) {
          console.error('❌ Ошибка добавления колонки table_type:', err.message);
        } else {
          console.log('✅ Колонка table_type добавлена в таблицу users');
        }
      });
    }
  });

  // Уникальный индекс на комбинацию user_id + guild_id + table_type
  db.run(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_user_guild_type
    ON users(user_id, guild_id, table_type)
  `);
  ...
  // =========================
  // TABLE ROLES (роли для таблиц small/medium/large)
  // =========================
  db.run(`
  ...
  `, (err) => {
    if (err) {
      console.error('❌ Ошибка создания таблицы table_roles:', err.message);
    } else {
      console.log('✅ Таблица table_roles готова');
    }
  });
  ...
  // =========================
  db.run(`
    CREATE TABLE IF NOT EXISTS x2_settings (
      guild_id TEXT PRIMARY KEY,
  ...
  `, (err) => {
    if (err) {
      console.error('❌ Ошибка создания таблицы x2_settings:', err.message);
    } else {
      console.log('✅ Таблица x2_settings готова');
    }
  });
  ...
  // =========================
  // 🔧 МИГРАЦИЯ (безопасная) для settings
  // =========================
  const columns = [
    'role1',
    'role2', 
